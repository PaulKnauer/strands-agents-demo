...
|---|---|---|---|---|---|---|
| R-1 | Prompt injection overrides the system prompt or causes the agent to behave outside its intended purpose | LOW | LOW | Narrow tool surface (one deterministic tool with no write access or external network calls); Bedrock Guardrails PROMPT_ATTACKS filter at HIGH strength (deployed in Epic 4, Story 4.3); automated red-team CI tests planned in Story 4.4 | Dev | Mitigated |
...
**R-1 (Prompt injection):** Mitigated by Bedrock Guardrails PROMPT_ATTACKS filter at HIGH strength (deployed in Story 4.3 via `deploy/guardrail.yaml`). Further evidence of mitigation to be generated by automated red-team CI tests in Story 4.4.
